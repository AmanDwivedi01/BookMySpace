...
<nav class="navbar navbar-inverse">
	<div class="container-fluid">
		<a class="navbar-brand" href="#">BookMySpace Ticketing Admin</a>
		<ul class="nav navbar-nav">
			<li class=""><a href="reservation.php">Booked
	...
	$('.tenPercent').click(function(event) {
		...
		$.ajax({
				...
				success: function (data) {
					// console.log(data);
					if(data.valid == false){
						$('#modal-message').find('.modal-body').text('Participant is Already Refunded!');
						
					}else if(data.valid == true){
						showAllTransaction();
						$('#modal-message').find('.modal-body').text('Participant Refunded Successfully!');
					}
